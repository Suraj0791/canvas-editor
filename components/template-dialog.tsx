...
export function TemplateDialog({
  ...
  const applyTemplate = (templateId: string) => {
    if (!canvas || !isCanvasReady) {
      console.error("[v0] Canvas is not ready")
      return
    }
  ...
  useEffect(() => {
    if (initialTemplate && canvas && isCanvasReady && !hasAppliedInitialTemplate.current) {
      hasAppliedInitialTemplate.current = true
      // Don't auto-apply, just log that we received an initial template
      console.log("[v0] Initial template specified:", initialTemplate)
    }
  }, [initialTemplate, canvas, isCanvasReady])
        ...
        <div className="grid grid-cols-2 gap-4 py-4">
          {templates.map((template) => (
            <button
              ...
              disabled={!canvas || !isCanvasReady}
              className="group flex flex-col gap-3 rounded-lg border-2 border-neutral-200 p-4 text-left transition-all hover:border-primary hover:shadow-md disabled:cursor-not-allowed disabled:opacity-50"
            >
